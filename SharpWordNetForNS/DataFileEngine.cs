...
//				}
        ...
        private class PosDataFileSet
        {
            ...
            {
                var _indexFileFs = new FileStream(Path.Combine(dataFolder, "index." + partOfSpeech), FileMode.Open, FileAccess.Read);
                _indexFile = new StreamReader(_indexFileFs);

                var _dataFileFs = new FileStream(Path.Combine(dataFolder, "data." + partOfSpeech), FileMode.Open, FileAccess.Read);

                _dataFile = new StreamReader(_dataFileFs);

                var _exceptionFileFs = new FileStream(Path.Combine(dataFolder, partOfSpeech + ".exc"), FileMode.Open, FileAccess.Read);
                _exceptionFile = new StreamReader(_exceptionFileFs);
            }
        }
